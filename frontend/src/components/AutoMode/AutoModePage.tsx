...
const AutoModePage: React.FC<AutoModePageProps> = ({
  projectName,
  ...
  const [activeAskUserMessage, setActiveAskUserMessage] =
    useState<Message | null>(null); // 当前活动的用户询问消息
  ...
  const [activeTab, setActiveTab] = useState<
    "messages" | "current-change" | "commits"
  >("messages"); // 修改标签状态
  ...
  const fetchCurrentChanges = async (current_event_file_id: string) => {
    try {
      ...
      // 构建请求URL，传递事件文件ID参数
      let url = `/api/current-changes?event_file_id=${encodeURIComponent(
        current_event_file_id
      )}`;
  ...
  useEffect(() => {
    ...
    autoCommandService.on("taskComplete", (isError: boolean) => {
      if (lastSubmittedQuery && currentEventFileId) {
        ...
        console.warn(
          "Cannot save task history: missing query or event file ID"
        );
      }
    });
  ...
  return (
    ...
    <div className={`flex-1 flex flex-col h-screen bg-gray-900 ${className}`}>
      ...
      <div
        className={`w-full max-w-4xl mx-auto px-4 py-6 flex flex-col ${
          ...
          <div className="flex-1 mb-6 flex flex-col items-center w-full overflow-hidden">
            ...
            <div
              className={`w-full max-h-full ${
                isMessageAreaAdaptive ? "max-w-full px-4" : "max-w-5xl"
              } flex`}
            >
              ...
              <div className="w-48 sticky top-0 self-start max-h-screen bg-gray-800 rounded-l-lg p-4 flex flex-col shadow-lg">
                ...
                <div className="space-y-3 mb-6">
                  <h3 className="text-gray-300 font-medium text-xs uppercase tracking-wider mb-2">
                    {getMessage("navigationMenu")}
                  </h3>
                  <button
                    className={`w-full px-3 py-2 text-left font-medium text-xs rounded-md flex items-center ${
                      activeTab === "messages"
                ...
                <div className="mt-auto flex flex-col space-y-2 pt-4 border-t border-gray-700">
                  <button
                    ...
                    onClick={() =>
                      setIsMessageAreaAdaptive(!isMessageAreaAdaptive)
                    }
                    title={
                      isMessageAreaAdaptive
                        ? getMessage("fullScreenWidth")
                        : getMessage("limitedWidth")
                    }
                  >
                    {isMessageAreaAdaptive ? (
                      <svg
              ...
              <div
                className={`flex-1 ${
                  isMessageAreaAdaptive ? "overflow-y-auto" : ""
                } bg-gray-800 rounded-r-lg p-5 shadow-lg`}
              >
                {/* 根据当前标签显示不同内容 */}
                {activeTab === "messages" ? (
                  <ChatPanel
