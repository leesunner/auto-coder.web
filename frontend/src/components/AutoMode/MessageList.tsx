...
const MessageItem = ({
  ...
  const renderMessageContent = (message: MessageProps) => {
    ...
    //     return <MarkdownMessage message={message} />;
    //   }
    console.log("Default text content ======= end");
    // Default text content
    return <MarkdownMessage message={message} />;
  };

  const aiDom = useMemo(() => renderMessageContent(message), [message.content]);
  ...
  const renderRotation = () => {
    ...
    if (message.type?.toUpperCase?.() === "COMPLETION") return null;

    return (
      <div className="pt-1">
        <RedoOutlined
          animate-spin
          className="animate-spin text-white flex-shrink-0"
        />
      </div>
    );
  };
    ...
    >
      ...
      >
        ...
        <div className="flex flex-row items-start">
          {message.isUser ? null : renderRotation()}
          <div className="flex-1 overflow-hidden">{aiDom}</div>
        </div>
...
const MessageList: React.FC<MessageListProps> = ({
  ...
  const filterMessages = (messages: MessageProps[]): MessageProps[] => {
    ...
    const filteredMessages = messagesWithoutLast.filter((message) => {
      // 过滤只有空字符的数据
      if (/^\s$/.test(message.content)) return false;
