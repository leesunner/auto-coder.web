...
def get_directory_tree(
    root_path: str, path: str = None, lazy: bool = False
) -> List[Dict[str, Any]]:
    """
...
async def get_directory_tree_async(
    root_path: str, path: str = None, lazy: bool = False, compact_folders: bool = False
) -> List[Dict[str, Any]]:
    """
    ...
    async def build_tree(current_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            ]  # Filter out None results from ignored items or errors
    ...
    def fn_compact_folders(
        nodes: List[Dict[str, Any]], parent_path: str = ""
    ) -> List[Dict[str, Any]]:
        def map_node(node: Dict[str, Any]) -> Dict[str, Any]:
            ...
            # 如果是目录且只有一个子目录，则合并路径
            if (
                not node.get("isLeaf", False)
                and "children" in node
                and len(node["children"]) == 1
                and not node["children"][0].get("isLeaf", False)
            ):
            ...
            return {
                **node,
                ...
                "children": (
                    fn_compact_folders(node["children"], current_path)
                    if "children" in node
                    else []
                ),
            }
                ...
                if lazy:
                    ...
                    if compact_folders:
                        if children.__len__() > 0:
                            child = children[0]
                            __is_dir__ = await aiofiles.os.path.isdir(
                                f"{full_path}/{child}"
                            )
...
def read_file_content(project_path: str, file_path: str) -> str:
    ...
    try:
        ...
        ) as f:  # Added errors='ignore' for robustness
            return f.read()
    except (
        IOError
    ):  # Catching only IOError, as UnicodeDecodeError is handled by errors='ignore'
...
async def read_file_content_async(project_path: str, file_path: str) -> Optional[str]:
    ...
    try:
        ...
        async with aiofiles.open(
            full_path, mode="r", encoding="utf-8", errors="ignore"
        ) as f:
